...
// ── Image Stack Block ──
function ImageStackBlock({ block, onUpdate, readOnly }) {
  const slides = block.slides || [{ id: Date.now().toString(), type: 'image', imageUrl: '', caption: '' }];
  const [currentIdx, setCurrentIdx] = useState(0);
  const [quizState, setQuizState] = useState({}); // { [slideId]: { selected, result } }
  ...
  const canGoNext = () => {
    ...
    // The CURRENT slide must not be a quiz that's still unsolved
    ...
    <div style={{ width: '100%', fontFamily: 'Roboto, sans-serif' }}>
      ...
      <div style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid #2a2a2a', minHeight: 200, background: '#111' }}>
        ...
        {currentSlide.type === 'image' && (
          <div>
            ...
            {/* Caption editor (author) / display (learner) */}
            {!readOnly ? (
              <div style={{ padding: '0.75rem', background: '#171717', borderTop: '1px solid #2a2a2a' }}>
                ...
                />
                {currentSlide.imageUrl && (
                  <label style={{ marginTop: '0.5rem', display: 'inline-flex', alignItems: 'center', gap: 4,
          ...
          <div style={{ padding: '1.25rem' }}>
            ...
            {!readOnly ? (
              <input
                ...
                style={{ width: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid #404040',
                  ...
                  <div key={oi} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    {readOnly ? (
                      <button
                        ...
                        style={{ flex: 1, textAlign: 'left', background: optBg, border: `1px solid ${optBorder}`,
                      ...
                      <>
                        <button
                          ...
                          style={{ flexShrink: 0, width: 20, height: 20, borderRadius: '50%', border: `2px solid ${isCorrectOpt ? '#4ade80' : '#404040'}`,
                        ...
                        <input
                          ...
                          style={{ flex: 1, background: '#1c1c1c', border: '1px solid #404040', color: '#d4d4d4',
            ...
            {!readOnly && (
              <button onClick={() => addOption(currentIdx)}
                style={{ marginTop: '0.75rem', background: 'transparent', border: '1px dashed #404040',
            ...
            {readOnly && qState.result === 'correct' && (
              <div style={{ marginTop: '1rem', background: '#052e16', border: '1px solid #166534', borderRadius: 6,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.75rem' }}>
        <button onClick={goPrev} disabled={currentIdx === 0}
          style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '0.4rem 0.9rem', borderRadius: 6,
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {slides.map((s, i) => (
            <div key={s.id}
              ...
              style={{ width: i === currentIdx ? 20 : 8, height: 8, borderRadius: 4,
      ...
      {!readOnly && (
        <div style={{ marginTop: '0.75rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button onClick={addImageSlide}
            style={{ background: '#1c1c1c', border: '1px solid #404040', color: '#d4d4d4',
          ...
          {slides.length > 1 && (
            <button onClick={deleteCurrentSlide}
              style={{ background: 'transparent', border: '1px solid #7f1d1d', color: '#ef4444',
...
function App() {
  ...
  const renderBlock = (block) => {
    switch (block.type) {
      ...
      case 'image-stack':
        return <ImageStackBlock block={block} onUpdate={updateBlock} readOnly={isPreviewOpen} />;
      case 'process':
        ...
        <div className="cf-layout">
          ...
          <aside className="cf-sidebar">
            <div className="cf-sidebar-section" style={{ opacity: activeSlideId ? 1 : 0.5, pointerEvents: activeSlideId ? 'auto' : 'none' }}>
              ...
              <button onClick={() => addBlock('image-stack')} className="cf-sidebar-btn">
                <Layers className="cf-sidebar-icon" /> Image Stack
              </button>
